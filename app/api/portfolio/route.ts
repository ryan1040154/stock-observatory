...
import {calculate,emptyPortfolio,validateTrade,type Portfolio} from '../../../lib/portfolio.ts';
export const dynamic='force-dynamic';
const response=(v:unknown,status=200)=>Response.json(v,{status,headers:{'Cache-Control':'no-store'}});
export async function GET(){try{const row=await database().prepare('SELECT payload,revision FROM portfolios WHERE user_id=?').bind('local').first<{payload:string;revision:number}>();return response({portfolio:row?JSON.parse(row.payload):emptyPortfolio(),revision:row?.revision??0});}catch{return response({error:'無法讀取庫存，請稍後重試。'},503);}}
export async function POST(request:Request){
 ...
 if(entries.size>4000)throw new Error('交易紀錄超過 4,000 筆。');p.trades=[...entries.values()];calculate(p.trades);p.watchlist=[...new Set<string>([...p.watchlist,...backup.watchlist])];if(p.watchlist.length>50)throw new Error('合併後自選超過 50 檔。');
 }
